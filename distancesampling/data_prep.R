...
################################################################
# crowd sourced data is subject to the whims of the person enterring the data
...
# mistakes in lat and long data entry can be hard to catch,
# some can be seen as outliers
hist(detections$`Detection Longitude`)
hist(detections$`Detection Latitude`)
...
# each count must be applied to 'replicate' we will treat each day a site was done as one replicate.
table(detections$`Transect ID`)
library(lubridate)
detections$jday<-yday(detections$Date)
detections$survey<-paste0(detections$jday,"-",detections$`Primary Observer initials`)

detections$replicate<-1
for(i in unique(detections$`Transect ID`)){
 detections$replicate[detections$`Transect ID`==i]<-as.numeric(as.factor(detections$survey[detections$`Transect ID`==i]))
}  

# replicate data need to be summarized by site do that below
...
# for each site, we need columns indicating which replicates a site was visited.  
repcols<-paste0("rep-",seq(1,max(detections$replicate)))

for(i in unique(sites$transect_id)){
  sites[sites$transect_id==i,repcols]<-rep(c(1:max(detections$replicate)) %in% detections$replicate[detections$`Transect ID`==i],
                                           each=sum(sites$transect_id==i))
}
sites$totalreps<-unlist(lapply(apply(st_drop_geometry(sites[,repcols]),1,which),which.max))
